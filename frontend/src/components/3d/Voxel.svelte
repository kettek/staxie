...
<T.Mesh scale={$scale} position={[position[0] + offset[0] + 0.5, position[1] + offset[1] + 0.5, position[2] + offset[2] + 0.5]} visible={!hidden} on:pointerover={hover} on:pointerout={leave} on:pointermove={move} on:pointerdown={down} on:pointerup={up} on:click={click}>
  <T.BoxGeometry />
  <T.MeshStandardMaterial transparent={opacity < 1} opacity={wireframe ? 0.2 : hovered ? 1 : opacity} color={hovered ? (0xffffff - realColor) | 0xff0000 : realColor} depthTest={!alwaysOnTop} depthWrite={!alwaysOnTop} />
  {#if wireframe}
